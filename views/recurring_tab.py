...
class RecurringTab(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.end_type_var = ctk.StringVar(value="없음")
        end_types = ["없음", "날짜 지정"]
    ...
    def update_end_options(self, event=None):
        ...
        if end_type == "없음":
            no_end_label = ctk.CTkLabel(self.end_options_frame, text="종료 날짜 없이 계속 반복됩니다.")
            no_end_label.pack(pady=5)
        else:  # 날짜 지정
            ...
            # 기본값: 3개월 후
            default_end = date.today() + datetime.timedelta(days=90)
    ...
    def add_recurring_meal(self):
        ...
        # 종료 날짜 확인
        end_date = None
        if self.end_type_var.get() == "날짜 지정":
            try:
        ...
        # 확인 메시지
        recur_type = self.recur_type_var.get()
        end_msg = "종료 없음" if end_date is None else f"종료: {end_date.strftime('%Y-%m-%d')}"
        
        message = f"{start_date.strftime('%Y-%m-%d')}부터 {recur_type} 반복 ({end_msg})으로\n"
